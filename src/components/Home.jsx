...
const Home = () => {
    ...
    const sumTotal = (shopCart) => {
        let total = 0
         if(carritoLength > 0){shopCart.cart.map((row,index) => total = total + +row.product.price)} 
         setSuma(total)
     }
    ...
    return(
        <Fragment>
            <Header shopCart={shopCart} restarTotal={restarTotal} borrarCarrito={borrarCarrito} suma={suma} sumTotal={sumTotal} carritoLength={carritoLength} ></Header>
